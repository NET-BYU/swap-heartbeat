...
void create_packet(char *packet, uint32_t seq_num, const char *src_mac, const char *dst_mac, const char *src_ip, const char *dst_ip, int src_port, int dst_port) {
...
int start_packet_transmission(
    ...
    char packet[sizeof(struct ethhdr) + sizeof(struct iphdr) + sizeof(struct tcphdr)];
    uint32_t seq_num = 0;
    struct timespec ts;
    ...
    for (int i = 0; i < num_packets; ++i) {
        ...
        if (time_diff < (i * interval)) {
            usleep((i * interval) - time_diff);
        }
        
        create_packet(packet, seq_num++, src_mac, dst_mac, src_ip, dst_ip, src_port, dst_port);
